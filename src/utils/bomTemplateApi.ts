// Update a single BOM template spec
export const updateBOMTemplateSpec = (specId: string, data: any) => {
  return axios.put(`${API_BASE_URL}/bom-template-spec/${specId}`, data);
};
...
export const createBOMTemplateSpecs = async (specs: any[]) => {
  ...
  const response = await axios.post(
    `${API_BASE_URL}/bom-template-spec/bulk`,
    payload
  );
  return response.data;
};
...
export const createBOMTemplateDetails = async (details: any[]) => {
  const payload = details.map((detail) => ({
    ...
    [BOM_TEMPLATE_DETAIL_KEY_MAP.materialType]:
      detail.materialType || "HIGH SIDE SUPPLY", // Default value
  }));

  const response = await axios.post(
    `${API_BASE_URL}/bom-template-detail/bulk`,
    payload
  );
  return response.data;
};
...
// Update BOM template
export const updateBOMTemplate = (id: string, data: any) => {
  return axios.put(`${API_BASE_URL}/bom-template/${id}`, data);
};
