...
import { CheckCircle, XCircle, Eye, Clock, AlertTriangle, Truck, Phone, Mail, MapPin, Tag } from 'lucide-react';

interface VendorApprovalProps {
  onApprovalAction: (vendorId: string, action: 'approve' | 'reject', reason?: string) => void;
}
  ...
  const pendingVendors = [
    {
      ...
      contactPersons: [
        { name: 'Vikram Singh', phone: '+91 98765 43211', email: 'vikram@bel.co.in' },
        { name: 'Priya Sharma', phone: '+91 98765 43212', email: 'priya@bel.co.in' }
      ],
      branches: [
        {
    ...
    },
    {
      ...
      bankName: 'HDFC Bank',
      contactPersons: [
        { name: 'Amit Patel', phone: '+91 87654 32110', email: 'amit@tatasteel.com' }
      ],
      branches: [
        {
    ...
    },
    {
      ...
      bankName: 'ICICI Bank',
      contactPersons: [
        { name: 'Arjun Mehta', phone: '+91 76543 21099', email: 'arjun@ashokleyland.com' },
        { name: 'Deepika Joshi', phone: '+91 76543 21100', email: 'deepika@ashokleyland.com' }
      ],
      branches: [
        {
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Pending Vendor Registrations</h3>
              <p className="text-sm text-gray-500">{pendingVendors.length} vendors awaiting approval</p>
            </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {pendingVendors.map((vendor) => (
                <tr key={vendor.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-1">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getVendorCategoryColor(vendor.vendorCategory)}`}>
                          {vendor.vendorCategory}
                        </span>
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getVendorTypeColor(vendor.vendorType)}`}>
                          {vendor.vendorType}
                        </span>
                      </div>
                  ...
                  <td className="px-6 py-4">
                    <div>
                      <p className="text-sm font-medium text-gray-900">{vendor.submittedBy}</p>
                      <p className="text-xs text-gray-500">
                        {new Date(vendor.submittedDate).toLocaleDateString('en-IN')}
                      </p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-4 max-h-[80vh] overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">Vendor Registration Details</h3>
              <button
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Vendor Category</label>
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getVendorCategoryColor(selectedVendor.vendorCategory)}`}>
                        {selectedVendor.vendorCategory}
                      </span>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Vendor Type</label>
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getVendorTypeColor(selectedVendor.vendorType)}`}>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{selectedVendor.city}, {selectedVendor.state} - {selectedVendor.pincode}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{selectedVendor.panNumber}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">GST Number</label>
                      <p className="text-sm text-gray-900">{selectedVendor.gstNumber}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Bank Name</label>
                      <p className="text-sm text-gray-900">{selectedVendor.bankName}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {selectedVendor.contactPersons.map((person: any, index: number) => (
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {selectedVendor.branches.map((branch: any, index: number) => (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {selectedVendor.uploadedFiles.map((file: any, index: number) => (
              ...
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {actionType === 'approve' ? 'Approve this vendor registration?' : 'Reject this vendor registration?'}
                  </p>
                </div>
              </div>
