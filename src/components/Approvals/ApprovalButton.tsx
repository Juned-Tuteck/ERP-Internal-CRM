...
export const ApprovalButton: React.FC<ApprovalButtonProps> = ({
    ...
    const [hierarchyExists, setHierarchyExists] = useState<boolean | null>(null);
    console.log("currentStatus", currentStatus);

    // Check if hierarchy exists on mount
    useEffect(() => {
        const checkHierarchy = async () => {
            const exists = await checkHierarchyExists(accessId);
    ...
    const handleSendForApproval = async () => {
        try {
            ...
            // Initiate approval (hierarchy already checked)
    ...
    const noHierarchy = hierarchyExists === false;

    return (
        <button
            onClick={handleSendForApproval}
            disabled={disabled || isLoading || isApprovalInProgress || isApproved || noHierarchy}
            title={
                noHierarchy ? 'No approval hierarchy configured' :
                    isApproved ? 'Already approved' :
                        isApprovalInProgress ? 'Approval already in progress' :
                            'Send for approval'
            }
            className={`inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${noHierarchy
